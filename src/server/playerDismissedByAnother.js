function countPlayerDismissed(deliveriesData) {
  const playerBowledCount = deliveriesData.filter(
    (deliveres) => deliveres['player_dismissed'] != '',
  );
  ...
  let sortedHighestNumberedDismissals = highestNumberedPlayerDismissals.sort(
    (a, b) => b.count - a.count,
  );
  const highestDismissal = sortedHighestNumberedDismissals[0].count;
  sortedHighestNumberedDismissals = sortedHighestNumberedDismissals.filter(
    (playerData) => playerData.count === highestDismissal,
  );
